//#region node_modules/.nitro/vite/services/ssr/assets/journal-BM2UEAXn.js
var HIST = [
	{
		key: "lt-2",
...
function analyzeJournal(rows, startEquity = 0) {
	...
	const chrono = [...closed].sort((a, b) => {
		return new Date(a.closed_at ?? a.opened_at).getTime() - new Date(b.closed_at ?? b.opened_at).getTime();
	});
	let cur = 0;
	let maxW = 0;
	let maxL = 0;
	...
	const curve = startEquity ? [{
		t: chrono[0] ? new Date(chrono[0].opened_at).getTime() : Date.now(),
		r: 0,
		usd: startEquity
	}] : [];
	...
	const byIndicator = /* @__PURE__ */ new Map();
...
function filterJournal(rows, q) {
	const sym = (q.symbol ?? "").trim().toUpperCase();
	return rows.filter((r) => {
		if (sym && !r.symbol.toUpperCase().includes(sym) && !r.symbol.replace("USDT", "").toUpperCase().includes(sym)) return false;
...
//#endregion
export { tradesToCsv as i, filterJournal as n, journalOptions as r, analyzeJournal as t };
