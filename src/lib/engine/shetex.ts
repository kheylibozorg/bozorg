...
/** TREX Entries from shetex.txt — native 4H lab, all chart TFs. */
export type ShetexProfile = {
  name: ShetexName;
  rr: number;
  legLookback: number;
  minLegAtr: number;
...
export const SHETEX_PROFILE: ShetexProfile = {
  name: "SHETEX",
  rr: 1.8,
  legLookback: 40,
  minLegAtr: 2.6,
...
export function shetexProfile(): ShetexProfile {
...
function moveOkUp(eOff: number, i: number, bars: Bar[], catr: number[], p: ShetexProfile) {
...
function moveOkDn(eOff: number, i: number, bars: Bar[], catr: number[], p: ShetexProfile) {
 ...
 */
export function runShetexEngine(bars: Bar[], tf: Timeframe, htfBars?: Bar[]): EngineResult {
