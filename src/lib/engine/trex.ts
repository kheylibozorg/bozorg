...
const EPS = 1e-10;
const BAND_ATR = 0.7;
const MIN_AWAY_ATR = 1.0;
const FTC_MIN_ATR = 0.2;

export type TrexProfile = {
  name: TrexName;
  rr: number;
  beAtR: number;
  legLookback: number;
...
export function trexProfile(name: TrexName): TrexProfile {
  return name === "TREX12" ? { ...TREX12 } : { ...TREX1 };
}

export function trexBeAtR(name: string): number | null {
...
function moveOkUp(eOff: number, i: number, bars: Bar[], catr: number[], p: TrexProfile) {
...
function moveOkDn(eOff: number, i: number, bars: Bar[], catr: number[], p: TrexProfile) {
...
export function runTrexEngine(
  ...
  for (let i = 2; i < n; i++) {
    ...
    const htfLongOk = !p.useHtfEma || !Number.isFinite(htfE) || htfC >= htfE * 0.997;
    ...
    if (atrOk) {
      const arm = (dir: 1 | -1, extreme: number, inner: number) => {
        ...
        const stopPctNow = (slDist / Math.max(ftc, EPS)) * 100;
        const tpPctNow = ((slDist * p.rr) / Math.max(ftc, EPS)) * 100;
        const ok =
          rm >= p.minRoomR &&
          i - lastFill >= p.cooldownBars &&
          stopPctNow >= p.minStopPct &&
          tpPctNow >= p.minTpPct;
        return { ok, ftc, sl };
      };
    ...
    lastBias =
      Number.isFinite(htfC) && Number.isFinite(htfE)
        ? htfC > htfE
          ? "LONG"
          : htfC < htfE
            ? "SHORT"
