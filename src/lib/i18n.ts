...
export const copy = {
  fa: {
    ...
    how: "نحوه استفاده",
    token: "توکن کرون",
    keys: "کلیدها فقط بعد از باز کردن پین ذخیره می‌شوند. API و سکرت و پرایوت‌کی در Neon رمزنگاری می‌شوند (سکرت). Test سبز فقط اتصال را چک می‌کند — زنده را با چک‌باکس Live روشن کن.",
  ...
  en: {
    ...
    how: "How to use",
    token: "Cron token",
    keys: "API keys save only after the operator PIN is unlocked. API, secret, and private keys are encrypted at rest in Neon. A green Test checks the connection; it does not arm live.",
